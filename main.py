...
import file_send.file_sender
from file_send import  file_sender
app = Flask(__name__)
UPLOAD_FOLDER = "file_send"
app.config['SERVER_NAME'] = '192.168.1.100:5545'
...
async def upload(channel_id):
    ...
    try:
        ...
        send = await file_send.file_sender.send_file(channel_id, auth, uploaded_file.filename)
        if(send == 200):
